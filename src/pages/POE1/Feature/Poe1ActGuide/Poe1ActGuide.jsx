...
useEffect(() => {
  ...
  const handleTimerReset = async () => {
    ...
    if (confirmed) { setIsTimerRunning(false); setElapsedTime(0); }
  };
  ...
  const handleDeleteAllRecords = async () => {
    ...
    if (confirmed) { setRecords([]); saveRecordsToLocalStorage([]); }
  };
      ...
      try {
        ...
        if (allItemIds.size > 0) {
          ...
          try {
            ...
            if (itemsResponse.ok) {
              allEnrichedItems = allEnrichedItems.concat(await itemsResponse.json());
            } else {
          ...
          try {
            ...
            if (currencyResponse.ok) {
              allEnrichedItems = allEnrichedItems.concat(await currencyResponse.json());
            } else {
  ...
  const handleAccordionClick = (clickedQuestId) => {
    setClosedQuestIds(prev =>
      prev.includes(clickedQuestId) ? prev.filter(id => id !== clickedQuestId) : [...prev, clickedQuestId]
    );
  };
  const handleSummaryItemClick = (clickedQuestId) => {
    const idx = quests.findIndex(q => q.id === clickedQuestId);
    if (idx === -1) return;
    const idsToClose = quests.slice(0, idx + 1).map(q => q.id);
  ...
  const renderDetailsWithTooltips = (quest, glossaryData) => {
    ...
    const options = {
      replace: (domNode) => {
        if (domNode.type === 'text' && domNode.data && domNode.data.includes('#ITEM#')) {
  ...
  const translations = {
    ko: {
      ...
      act1: '1막', act2: '2막', act3: '3막', act4: '4막', act5: '5막',
      act6: '6막', act7: '7막', act8: '8막', act9: '9막', act10: '10막',
      error: '에러 발생:', error_guide: 'Local WP가 켜져 있는지, 주소가 정확한지, CORS 문제가 없는지 확인하세요.',
      recordBoard: 'My Speedrun Records', noRecordsTitle: '저장된 기록이 없습니다',
      noRecordsText: '아직 기록이 없네요? 아래처럼 당신만의 기록을 완성하세요!',
      date: '날짜', act: '액트', time: '시간', delete: '삭제', deleteAll: '전체 삭제',
      ok: '확인', cancel: '취소', confirmResetTimer: '타이머를 초기화하시겠습니까?',
      confirmResetProgress: '정말로 진행 상황을 초기화하시겠습니까?',
      confirmDeleteRecord: '이 기록을 삭제하시겠습니까?',
      confirmDeleteAllRecords: '정말로 모든 기록을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.',
    ...
    en: {
      ...
      act1: 'Act 1', act2: 'Act 2', act3: 'Act 3', act4: 'Act 4', act5: 'Act 5',
      act6: 'Act 6', act7: 'Act 7', act8: 'Act8', act9: 'Act9', act10: 'Act 10',
      error: 'An error occurred:', error_guide: 'Please check if Local WP is running, the address is correct, and there are no CORS issues.',
      recordBoard: 'My Speedrun Records', noRecordsTitle: 'No Records Saved',
      noRecordsText: 'You dont have any record! Make your own record like below!',
      date: 'Date', act: 'Act', time: 'Time', delete: 'Delete', deleteAll: 'Delete All',
      ok: 'OK', cancel: 'Cancel', confirmResetTimer: 'Are you sure you want to reset the timer?',
      ...
      promptMemo: 'Enter a memo for this record (optional):', alertRecordSaved: 'Record saved.'
    }
  };
    ...
    <div className="act-guide-container">
      ...
      <main className="guide-main-content">
          ...
          <div className="timer-accordion-wrapper">
            ...
            <div className={`accordion-content ${isTimerAccordionOpen ? 'open' : ''}`}>
              ...
              <div className="timer-record-buttons">
                <button onClick={openRecordModal} disabled={isTimerRunning || elapsedTime === 0}>{t.save}</button>
          ...
          <aside className="summary-box">
            ...
            <ul>
              {quests.map(quest => {
                ...
                return (
                  <li key={quest.id} onClick={() => handleSummaryItemClick(quest.id)}>
        ...
        <section className="details-section">
          <div className="act-tabs-container">
          ...
          <div className="details-content-wrapper" ref={detailsContentRef}>            
            ...
            <div className="accordion-container">
              {quests.map(quest => {
                ...
                return (
                  <div key={quest.id} className="accordion-item">
                    ...
                    <div className={`accordion-content ${isOpen ? 'open' : ''}`}>
                      {renderDetailsWithTooltips(quest, glossaryData)}
                    </div>
                  </div>
                );
              })}
        ...
        {records.length > 0 ? (
            ...
            <table>
              ...
              <tbody>
                {records.map((record, index) => (
                  <tr key={record.id} className={index === 0 ? 'is-first-place' : ''}>
                    <td># {index + 1}</td>
                    <td>{record.time}</td>
                    <td onClick={() => handleMemoClick(record)}>
                      {editingRecordId === record.id ? (
                        <input
                          ...
                          onBlur={() => handleMemoSave(record.id)}
                          onKeyDown={(e) => e.key === 'Enter' && handleMemoSave(record.id)}
                          autoFocus
          ...
          <div className="record-board-placeholder">
            ...
            {uiIcons.Speedrun_Example && (
              <img
                src={uiIcons.Speedrun_Example}
                alt="Example Records Chart"
